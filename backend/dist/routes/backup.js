...
router.post('/import', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        for (const a of accounts) {
            if (!a.id || !a.name)
                continue;
            await prisma.account.upsert({ where: { id: a.id }, update: { name: a.name, currency: a.currency || 'USD', defaultFeePerMiniContract: a.defaultFeePerMiniContract ? Number(a.defaultFeePerMiniContract) : null, defaultFeePerMicroContract: a.defaultFeePerMicroContract ? Number(a.defaultFeePerMicroContract) : null }, create: { id: a.id, name: a.name, currency: a.currency || 'USD', userId: req.userId, defaultFeePerMiniContract: a.defaultFeePerMiniContract ? Number(a.defaultFeePerMiniContract) : undefined, defaultFeePerMicroContract: a.defaultFeePerMicroContract ? Number(a.defaultFeePerMicroContract) : undefined } });
        }
        const accountFees = await readCsv('accountFees.csv');
        for (const f of accountFees) {
            if (!f.accountId || !f.assetClass)
                continue;
            await prisma.accountFee?.upsert?.({ where: { accountId_assetClass: { accountId: f.accountId, assetClass: f.assetClass } }, update: { mode: f.mode, value: Number(f.value || 0) }, create: { accountId: f.accountId, assetClass: f.assetClass, mode: f.mode, value: Number(f.value || 0) } });
        }
        const trades = await readCsv('trades.csv');
        for (const t of trades) {
            if (!t.id || !t.accountId || !t.symbol)
                continue;
            await prisma.trade.upsert({ where: { id: t.id }, update: { accountId: t.accountId, symbol: t.symbol, assetClass: t.assetClass || 'STOCK', size: Number(t.size || 0), entryPrice: Number(t.entryPrice || 0), entryTime: t.entryTime ? new Date(t.entryTime) : new Date(), exitPrice: t.exitPrice ? Number(t.exitPrice) : null, exitTime: t.exitTime ? new Date(t.exitTime) : null, fees: t.fees ? Number(t.fees) : null, stopPrice: t.stopPrice ? Number(t.stopPrice) : null, targetPrice: t.targetPrice ? Number(t.targetPrice) : null, strategy: t.strategy || null, notes: t.notes || null, confidence: t.confidence ? Number(t.confidence) : null, setupMode: t.setupMode === 'true' }, create: { id: t.id, userId: req.userId, accountId: t.accountId, symbol: t.symbol, assetClass: t.assetClass || 'STOCK', size: Number(t.size || 0), entryPrice: Number(t.entryPrice || 0), entryTime: t.entryTime ? new Date(t.entryTime) : new Date(), exitPrice: t.exitPrice ? Number(t.exitPrice) : null, exitTime: t.exitTime ? new Date(t.exitTime) : null, fees: t.fees ? Number(t.fees) : null, stopPrice: t.stopPrice ? Number(t.stopPrice) : null, targetPrice: t.targetPrice ? Number(t.targetPrice) : null, strategy: t.strategy || null, notes: t.notes || null, confidence: t.confidence ? Number(t.confidence) : null, setupMode: t.setupMode === 'true' } });
        }
        const transactions = await readCsv('transactions.csv');
        for (const tx of transactions) {
            if (!tx.id || !tx.accountId || !tx.type || !tx.amount)
                continue;
            await prisma.transaction.upsert({ where: { id: tx.id }, update: { accountId: tx.accountId, type: tx.type, amount: Number(tx.amount), currency: tx.currency || 'USD', createdAt: tx.createdAt ? new Date(tx.createdAt) : new Date() }, create: { id: tx.id, accountId: tx.accountId, type: tx.type, amount: Number(tx.amount), currency: tx.currency || 'USD', createdAt: tx.createdAt ? new Date(tx.createdAt) : new Date() } });
        }
        const tags = await readCsv('tags.csv');
        for (const tg of tags) {
            if (!tg.id || !tg.name)
                continue;
            await prisma.tag.upsert({ where: { id: tg.id }, update: { name: tg.name }, create: { id: tg.id, userId: req.userId, name: tg.name } });
        }
        const settingsRows = await readCsv('settings.csv');
        for (const s of settingsRows) {
            if (!s.id)
                continue;
            await prisma.userSettings.upsert({ where: { id: s.id }, update: { favoriteAccountId: s.favoriteAccountId || null, defaultChartInterval: s.defaultChartInterval || null, defaultChartWindowDays: s.defaultChartWindowDays ? Number(s.defaultChartWindowDays) : null }, create: { id: s.id, userId: req.userId, favoriteAccountId: s.favoriteAccountId || null, defaultChartInterval: s.defaultChartInterval || null, defaultChartWindowDays: s.defaultChartWindowDays ? Number(s.defaultChartWindowDays) : null } });
        }
